...
@Component({
  selector: "ngx-report-beli",
  templateUrl: "./report.beli.component.html"
})
export class ReportBeliComponent {
  @ViewChild("myForm")
  ...
  getDataReport() {
    this.service
      .getreq("report_beli_barangs")
      .toPromise()
      .then(response => {
        if (response != null) {
          console.log(response);
          response.forEach(element => {
            element.TANGGAL_BELI = moment(element.TANGGAL_BELI).format(
              "DD-MM-YYYY"
            );
          });
